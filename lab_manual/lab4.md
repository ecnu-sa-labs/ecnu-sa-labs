...
```tex
File and Function: test1.py::test_find_vaild
...
E   assert False
    ...
    Falsifying example: test_find_vaild(
        key=0,
...
def test_insert_vaild(key: int, value: int, bst: BST[int,int]) -> None:
    # assume(is_valid(bst))
...
def test_delete_vaild(key: int, bst: BST[int,int]) -> None:
    # assume(is_valid(bst))
...
🔬 运行属性测试 (test1.py)...
...FF
FAILED test1.py::test_find_vaild - assert False
FAILED test1.py::test_union_vaild - assert False
...
For `union`: To verify the outcome of the delete operation, one may establish a relationship by **adding an insert**. **Please note that we have not introduced bugs into the union, but the metamorphic properties of this union are prone to errors in implementation. Correct properties will not detect bugs.**
